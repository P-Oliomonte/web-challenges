...
const inputSliderColor = document.querySelector('[data-js="input-color"]');
const inputSliderRadius = document.querySelector('[data-js="input-radius"]');
const inputSliderRotation = document.querySelector(
  '[data-js="input-rotation"]'
);
const box = document.querySelector('[data-js="box"]');

inputSliderColor.addEventListener("change", () => {
  const newBackgroundColor = `hsl(${inputSliderColor.value} 70% 60%)`;
  box.style.setProperty("background", newBackgroundColor);
});

inputSliderRadius.addEventListener("change", () => {
  const newRadius = `${inputSliderRadius.value}%`;
  box.style.setProperty("border-radius", newRadius);
});

inputSliderRotation.addEventListener("change", () => {
  const newRotation = `rotate(${inputSliderRotation.value}deg)`;
  box.style.setProperty("transform", newRotation);
});
